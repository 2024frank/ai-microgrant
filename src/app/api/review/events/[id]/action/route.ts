...
export async function POST(
  ...
  // Approve/resubmit: allowed from most statuses, but never while a fix request is active.
  ...
  if (action === 'approve' && event.status === 'pending_fix') {
    return Response.json({ error: 'Event is awaiting correction and cannot be approved yet' }, { status: 409 });
  }
