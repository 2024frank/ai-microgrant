...
Keep following the "Next" link until there is no next page. Each card shows a title, a date and time, an address, and a "Read More" link. Capture the exact "Read More" href for every card - that is the event's own detail-page URL (for example https://oberlin-public-library.locable.com/2026/07/15/558359/l-e-g-o/). You will need it in every step below.

## STEP 2 - Open each distinct program's detail page (MANDATORY)
The index cards carry NO description and NO image. You MUST fetch the detail page for each DISTINCT program before returning it - never build an event from the index card alone. There are only about seven to ten distinct programs (the fifty-one listings are mostly recurring occurrences of Storytime and L.E.G.O.), so this is a small number of fetches, not fifty. From each detail page read:
...
- the event image: read the page's og:image meta tag and set image_cdn_url to that public https URL (the value looks like https://images.locable.com/...). Every Locable detail page has one and it is the event's real image. Only if a page genuinely exposes no og:image, omit image_cdn_url and record why in fieldNotes.image_cdn_url.
...
- calendarSourceUrl and website: REQUIRED - set BOTH to the exact "Read More" detail-page URL you captured in Step 1 for that program (for example https://oberlin-public-library.locable.com/2026/07/15/558359/l-e-g-o/). Never the index page, never a guessed or constructed URL, never left empty. The platform re-reads this page to recover the image, so it must be the real per-event detail page.
...
async function main() {
  ...
  if (system.includes('Open each distinct program\'s detail page (MANDATORY)') && !system.includes('whofi.com')) {
    console.log('Already on the latest Locable contract; no change. version:', current.version);
    return;
  }
